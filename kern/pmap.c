...
void page_init(void) {
	...
	for (int i = init_pages; i < npage; i++) {
		pages[i].pp_ref = 0;
...
static int pgdir_walk(Pde *pgdir, u_long va, int create, Pte **ppte) {
	...
	if (!(*pgdir_entryp & PTE_V)) {
		if (create) {
			...
			*pgdir_entryp = page2pa(pp); // Set physic address.
			*pgdir_entryp = (*pgdir_entryp) | PTE_V | PTE_D; // Set valid and writable bits.
			pp->pp_ref++;
		} else {
	 ...
	 for (int i = 0; i < BY2PG>>2; i++) {
		...
		for (int j = 0; j < BY2PG>>2; j++) {
			...
			if ((perm_mask & PTE_COW) && (*temp2 & PTE_COW) == 0)
				continue;
			if ((perm_mask & PTE_N) && (*temp2 & PTE_N) == 0)
				continue;
